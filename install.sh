...
if ! is_container; then
    ...
    if ! command -v fish >/dev/null 2>&1; then
        ...
        /usr/bin/env fish << 'EOF'
            ...
            # Configure tide prompt (TERM=dumb prevents screen clear)
            TERM=dumb tide configure --auto --style=Classic --prompt_colors='True color' --classic_prompt_color=Light --show_time=No --classic_prompt_separators=Angled --powerline_prompt_heads=Sharp --powerline_prompt_tails=Flat --powerline_prompt_style='Two lines, character' --prompt_connection=Disconnected --powerline_right_prompt_frame=No --prompt_spacing=Sparse --icons='Many icons' --transient=Yes
EOF
    fi # End of fish
